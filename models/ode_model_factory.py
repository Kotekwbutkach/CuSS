...
def cucker_smale_function_factory(phi: Callable[[np.ndarray], np.ndarray],
    ...
    def cucker_smale_function(state: np.ndarray):
        ...
        distance_in_x = distance(positions.transpose((0, 3, 1, 2))[0, :], positions.transpose((0, 3, 2, 1))[0, :])
...
class OdeModelFactory:
    ...
    def create_standard(
            ...
            distance: Callable[[np.ndarray, np.ndarray], np.ndarray]):
        return OdeModel(time_step, cucker_smale_function_factory(phi, distance))
