...
class ParticlesSystem:
    ...
    def __init__(self,
        ...
        if particles is None:
            self.particles = np.zeros((number_of_particles, step_limit, 3))
        else:
    ...
    def at_step(self, step: int):
        ...
        return ParticlesState(self.number_of_particles, self.particles[:, step, :])

    def steps(self):
        _s = 0

        while _s < self.step_limit:
            yield ParticlesState(self.number_of_particles, self.particles[:, _s, :])
            _s += 1
